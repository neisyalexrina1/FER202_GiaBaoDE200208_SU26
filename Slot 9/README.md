...
-- | -- | -- | -- | -- | --
1 | DE200208 | Đỗ Minh Gia Bảo | dominhgiabaobmg@gmail.com | Leader | https://github.com/neisyalexrina1/FER202_GiaBaoDE200208_SU26.git
2 | DE190049 | Nguyễn Duy Quang | kingdn522005@gmail.com | Developer | https://github.com/kingDlight/FER202_QuangND_SU26.git
3 | DE190656 | Huỳnh Minh Huân | huynhhuan456@gmail.com | Developer | https://github.com/HMSFLASH/FER202_HuanMinh_SU26.git
...
- [x] Tạo `ThemeContext`, `ThemeProvider`
- [x] State `mode`: đọc từ `localStorage` khi khởi tạo, mặc định `'system'`
- [x] State `systemPrefersDark`: đọc từ `window.matchMedia`
- [x] `useEffect`: lắng nghe thay đổi OS theme, cleanup khi unmount
- [x] `resolvedTheme`: tính từ mode + systemPrefersDark
- [x] `changeMode(newMode)`: set mode, lưu vào localStorage
- [x] Dùng `useMemo` cho value để tránh re-render thừa
- [x] Export custom hook `useTheme()`

#### `src/components/theme/ThemeNavbar.jsx`
- [x] Dùng `useTheme()` lấy `mode`, `resolvedTheme`, `colors`, `changeMode`
- [x] Render 3 nút từ `THEME_MODES` (import từ `themeConfig`)
- [x] Highlight nút đang active, hiển thị `resolvedTheme`
- [x] Áp dụng màu từ `colors`, không nhận màu qua props

#### `src/components/theme/ThemedCard.jsx`
- [x] Dùng `useTheme()` lấy `colors`
- [x] Props: `title`, `children`
- [x] Áp dụng `colors.surface`, `colors.border`

#### `src/components/theme/ThemedButton.jsx`
- [x] Dùng `useTheme()` lấy `colors`
- [x] Props: `children`, `onClick`, `variant` ('primary' | 'outline')
- [x] Áp dụng màu theo variant từ colors

#### `src/components/theme/ThemedInput.jsx`
- [x] Dùng `useTheme()` lấy `colors`
- [x] Props: `placeholder`
- [x] Áp dụng `colors.background`, `colors.border`, `colors.text`

#### `src/pages/Ex04ThemePage.jsx`
- [x] Bọc trong `<ThemeProvider>`
- [x] Tạo component nội bộ `ThemePageContent` dùng `useTheme()` để set background/color wrapper
- [x] Render `<ThemeNavbar />` và ít nhất 3 `<ThemedCard>` chứa các component khác
